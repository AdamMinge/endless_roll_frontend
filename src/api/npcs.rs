...
pub async fn get_campaign_npcs(campaign_id: &str) -> Result<Vec<Npc>, String> {
    let token = get_auth_header()?;

    let response = Request::get(&format!("{}/campaigns/{}/npcs", API_URL, campaign_id))
